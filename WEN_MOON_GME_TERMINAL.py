...
<style>
    ...
    .title-container { text-align: center; margin-bottom: 20px; margin-top: 45px; }
...
if not st.session_state.launched and not st.session_state.show_leaderboard:
    ...
    with st.container():
        ...
        st.markdown("<hr style='border: 1px solid #00FF00; margin-top: 40px; margin-bottom: 30px;'>", unsafe_allow_html=True)
        st.markdown("<h2 style='text-align:center; color:#00FF00; font-family:monospace;'>📥 INSTITUTIONAL REPORT</h2>", unsafe_allow_html=True)
            ...
            try:
                ...
                try:
                    ...
                    if os.path.exists("logo_gamestop.png"):
                        pdf.image("logo_gamestop.png", x=15, y=12, w=40)
                except: pass
                
                pdf.set_font("Arial", "B", 22)
                pdf.cell(0, 15, "GAMESTOP (GME) TEAR SHEET", ln=1, align="C")
                ny_time = datetime.now(pytz.timezone('America/New_York')).strftime('%Y-%m-%d %H:%M:%S EST')
                ...
                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 8, " EPS & REVENUE CONSENSUS", ln=1, fill=True)
                pdf.set_font("Arial", "B", 10)
                headers = ["Period", "Est EPS", "Act EPS", "Est Rev", "Act Rev"]
                w = [30, 35, 35, 40, 40]
                for i in range(5): pdf.cell(w[i], 8, headers[i], border=1, align="C")
                pdf.ln()
                
                pdf.set_font("Arial", "", 10)
                h_data = [
                    ["FY 2025", "$0.84", "$1.18", "$4.16B", "$3.63B"],
                    ["FY 2024", "$0.07", "$0.33", "$4.02B", "$3.82B"],
                    ["FY 2023", "$0.13", "$0.06", "$5.53B", "$5.27B"]
                ]
                for r in h_data:
                    for i in range(5): pdf.cell(w[i], 8, r[i], border=1, align="C")
                    pdf.ln()
                pdf.ln(5)

                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 8, " NET & OPERATING INCOME (FY)", ln=1, fill=True)
                pdf.set_font("Arial", "B", 10)
                pdf.cell(60, 8, "Year", border=1, align="C")
                pdf.cell(60, 8, "Net Income", border=1, align="C")
                pdf.cell(60, 8, "Operating Income", border=1, align="C", ln=1)
                
                pdf.set_font("Arial", "", 10)
                i_data = [["2025", "$418.4M", "$232.1M"], ["2024", "$131.3M", "-$19.1M"], ["2023", "$6.7M", "-$28.6M"]]
                for r in i_data:
                    pdf.cell(60, 8, r[0], border=1, align="C")
                    pdf.cell(60, 8, r[1], border=1, align="C")
                    pdf.cell(60, 8, r[2], border=1, align="C", ln=1)
                pdf.ln(5)

                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 8, " STORE EFFICIENCY", ln=1, fill=True)
                pdf.set_font("Arial", "B", 10)
                pdf.cell(60, 8, "Year", border=1, align="C")
                pdf.cell(60, 8, "Total Stores", border=1, align="C")
                pdf.cell(60, 8, "Revenue Per Store", border=1, align="C", ln=1)
                pdf.set_font("Arial", "", 10)
                s_data = [["2025", "2,206", "$1.65M"], ["2024", "3,203", "$1.19M"], ["2023", "4,169", "$1.26M"]]
                for r in s_data:
                    pdf.cell(60, 8, r[0], border=1, align="C")
                    pdf.cell(60, 8, r[1], border=1, align="C")
                    pdf.cell(60, 8, r[2], border=1, align="C", ln=1)
        ...
        if pdf_bytes:
            ...
            st.warning("⚠️ LIBRARIES MISSING: Please run 'pip install fpdf2 pytz' in your environment to enable PDF generation.")
