...
# INITIALISATION PROPRE (Pour éviter les erreurs)
...
# --- FONCTION DE MISE À JOUR SANS ERREUR ROUGE ---
def update_portfolio_logic():
    nsq = st.session_state.in_nsq
    nsp = st.session_state.in_nsp
    nwq = st.session_state.in_nwq
    nwp = st.session_state.in_nwp
...
# --- 2.5 LEADERBOARD SCREEN ---
    ...
    u_sq, u_wq = st.session_state.get("osq", 0), st.session_state.get("owq", 0)
...
# --- 3. TERMINAL & LIVE ENGINE ---
    ...
    # --- START OF LIVE FRAGMENT (THE ANTI-DUPLICATION CORE) ---
    @st.fragment(run_every="30s")
    def render_content():
        plt.close('all') # ULTIMATE CACHE KILLER
        
        # 1. LIVE DATA FETCHING INSIDE FRAGMENT
        p_nsy, p_wt, pr_nsy, pr_wt, ch_gme, ch_wt = fetch_terminal_data()
        
        # 2. PERSISTENT USER DATA (Will never erase on return)
        ...
        # 3. TABS DECLARED INSIDE FRAGMENT (Kills infinite duplication)
        ...
        with tab3:
            ...
            with ph3.container():
                ...
                ac = fig4.add_subplot(gs[1]); ac.set_facecolor("#0f172a"); ac.axis('equal') # KEEP IT PERFECTLY ROUND
                wedges, texts = ac.pie(
                ...
                # DYNAMIC ARROWS WITHOUT SQUISHING THE CHART
                ac.annotate(f"{pct_s:.0f}%", xy=(-1.3, 0), xytext=(-2.2, 0),
                            color="#4ade80", fontsize=50, weight="bold", ha="center", va="center",
                            arrowprops=dict(arrowstyle="-", color="#4ade80", lw=5))
                ac.annotate(f"{pct_w:.0f}%", xy=(1.3, 0), xytext=(2.2, 0),
        ...
        with tab5:
            ...
            with ph5.container():
                ...
                ac_c.annotate(f"{cpct_s:.0f}%", xy=(-1.3, 0), xytext=(-2.2, 0),
                            color="#4ade80", fontsize=50, weight="bold", ha="center", va="center",
                            arrowprops=dict(arrowstyle="-", color="#4ade80", lw=5))
                ac_c.annotate(f"{cpct_w:.0f}%", xy=(1.3, 0), xytext=(2.2, 0),
                            color="#0f5132", fontsize=50, weight="bold", ha="center", va="center",
                            arrowprops=dict(arrowstyle="-", color="#0f5132", lw=5))
        ...
        # --- 8TH TAB : WEN MOON SUMMARY (ERROR-FREE) ---
        with tab8:
            ph8 = st.empty()
            with ph8.container():
                u_name = st.session_state.get("ape_name", "Anonymous")
