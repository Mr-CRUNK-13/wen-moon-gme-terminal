...
# --- 3. TERMINAL & LIVE ENGINE ---
    ...
    def render_content():
                ...
                with t:
                    if idx >= 3: # 🟣 DRS TABS (3, 4, 5)
                        ...
                        for i, r in enumerate(s_db):
                            ...
                            html += f"<tr><td>{rnk}</td><td><span {n_style}>{r['name']}</span></td><td style='color:#9b51e0;'>${r.get(sort_k, 0):,.2f}</td><td>{r.get('drs_sq', 0):,}</td><td>{r.get('drs_wq', 0):,}</td><td style='font-size:20px;'>{trphs}</td></tr>"
                        st.markdown(html + "</table></div>", unsafe_allow_html=True)
                    else: # 🌍 STANDARD TABS (0, 1, 2)
                        html = "<div class='table-wrapper'><table class='ldb-t'><tr><th>RANK</th><th>APE</th><th>TOTAL VALUE</th><th>SHARES</th><th>WARRANTS</th><th>TROPHIES</th></tr>"
        ...
        with ph8.container():
            ...
            html_summary = f"""
            <div style="background-color: #0f172a; padding: 20px; border-radius: 10px; border: 1px solid #0259c7; box-shadow: 0 0 15px rgba(2, 89, 199, 0.3);">
                ...
                <div style="display: flex; justify-content: space-around; flex-wrap: wrap; margin-bottom: 30px; gap: 20px;">
                    ...
                    <div style="background-color: #0e1621; padding: 20px; border-radius: 8px; border: 2px solid #006400; flex: 1; min-width: 250px;">
                        <h4 style="color: #006400; margin-top: 0; font-size: 24px; font-weight: bold;">🟢 WARRANTS</h4>
