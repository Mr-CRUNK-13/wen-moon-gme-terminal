...
# --- CONFIGURATION ÉCRAN ---
st.set_page_config(page_title="WEN MOON TERMINAL V61", layout="wide", initial_sidebar_state="collapsed")

# --- DESIGN NÉON INSTITUTIONNEL (CSS) ---
st.markdown("""
<style>
...
# --- MOTEUR D'ASPIRATION DE DONNÉES ---
...
# --- SIDEBAR : CONTRÔLE DU PORTefeuille ---
with st.sidebar:
...
# Calculs communautaires (Simulés pour la démo V61)
c_s, c_w = qn, qw 
...
st.markdown(f"<h1 style='text-align: center; color: #00FF00; font-family: monospace;'>💎 WEN MOON TERMINAL V61 💎</h1>", unsafe_allow_html=True)

def render_content():
    # Grille des écrans LED
    ...
    ph9 = st.empty() # GME PRO
        ...
        with ph8.container():
            ...
            # Récupération des données saisies dans la sidebar
        ...
        with ph9.container():
            ...
            with ptab3:
                # Tableau historique sur-mesure (GME Net Income)
        ...
        with ph10.container():
            ...
            if opts:
                selected_expiry = st.selectbox("📅 EXPIRATION DATE:", opts, key="opt_exp_selector")
                try:
                    tk = yf.Ticker("GME")
                    chain = tk.option_chain(selected_expiry)
                    # Logique de centrage (12 strikes +/-)
                ...
                except: st.warning("Options feed unavailable.")
        ...
        with ph11.container():
            ...
            with ctab2:
                if not fin.empty: st.dataframe(fin.iloc[:10, :3].astype(str), use_container_width=True)

        with ph12.container():
            st.markdown("<h2 style='text-align:center; color:#00FF00; font-family:monospace;'>🕵️ INSIDERS</h2>", unsafe_allow_html=True)
            if not ins.empty: st.dataframe(ins.head(10).astype(str), use_container_width=True)
