...
# --- 3. TERMINAL & LIVE ENGINE ---
    ...
    def render_content():
        ...
        with ph3.container():
            ...
            with st.container():
                st.markdown('<div id="p3-neon-anchor" style="display:none;"></div><style>div[data-testid="stVerticalBlock"]:has(> div #p3-neon-anchor) { border: 2px solid #00FF00 !important; box-shadow: 0 0 15px #00FF00 !important; border-radius: 15px !important; padding: 10px !important; background: #0e1621 !important; margin-bottom: 20px !important; }</style>', unsafe_allow_html=True)
        ...
        with ph5.container():
            ...
            with st.container():
                st.markdown('<div id="p5-neon-anchor" style="display:none;"></div><style>div[data-testid="stVerticalBlock"]:has(> div #p5-neon-anchor) { border: 2px solid #00FF00 !important; box-shadow: 0 0 15px #00FF00 !important; border-radius: 15px !important; padding: 10px !important; background: #0e1621 !important; margin-bottom: 20px !important; }</style>', unsafe_allow_html=True)
                ...
                with t:
                    if idx >= 3:
                        ...
                        for i, r in enumerate(s_db):
                            ...
                            html += f"<tr><td>{rnk}</td><td><span {n_style}>{r['name']}</span></td><td style='color:#9b51e0;'>${r.get(sort_k, 0):,.2f}</td><td>{r.get(q_s_key, 0):,}</td><td>{r.get(q_w_key, 0):,}</td><td style='font-size:20px;'>{trphs}</td></tr>"
                        st.markdown(html + "</table></div>", unsafe_allow_html=True)
                    else: # 🌍 STANDARD TABS (0, 1, 2)
                        html = "<div class='table-wrapper'><table class='ldb-t'><tr><th>RANK</th><th>APE</th><th>TOTAL VALUE</th><th>SHARES</th><th>WARRANTS</th><th>TROPHIES</th></tr>"
                        ...
                        for i, r in enumerate(s_db):
                            rnk = "🥇" if i == 0 else "🥈" if i == 1 else "🥉" if i == 2 else str(i+1)
                            trphs = "🟣" if r.get('drs_tv', 0) > 0 else ""
                            html += f"<tr><td>{rnk}</td><td><strong>{r['name']}</strong></td><td style='color:#00FF00;'>${r.get('tv', 0):,.2f}</td><td>{r.get('sq', 0):,}</td><td>{r.get('wq', 0):,}</td><td style='font-size:20px;'>{trphs}</td></tr>"
                        st.markdown(html + "</table></div>", unsafe_allow_html=True)
